...
var readability = {
    ...
    minimumArticleContentSize: 250, /* minimum acceptable content size*/
        /**
    ...
    grabArticle: function (page) {
         ...
         **/
        if(readability.getInnerText(articleContent, false).length < readability.minimumArticleContentSize) {
...
exports.parse = function parse(theHtml, url, options, callback) {
	...
	if (options.minContentSize) readability.minimumArticleContentSize = options.minContentSize;
	...
	try {
		...
		function createDocWithHTMLParser() {
			var doc = (jsdom.jsdom.length == 2 ? jsdom.jsdom(html, docOptions) : jsdom.jsdom(html, null, docOptions));
			return doc;
		}
		...
		if (!doc.body) {
			...
			return callback({title: '', content: '', document: '', error: true});
		}
		...
		start(win, options, function(html) {
			...
			callback({title: document.title, content: html, document: doc, time: time / 1000, inputLength: theHtml.length});
            win && win.close();
			win = null;
			doc = null;
		});
		...
		callback({title: '', content: '', document: '', error: true});
	}
};
